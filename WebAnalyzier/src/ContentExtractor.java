...
public class ContentExtractor {
	...
	public List<String> parse() throws IOException {
		...
		//remove stopwords
		List<String> titleClean = stemming(title);
		//clean and uniform the text
		String text = doc.body().text().toLowerCase().replaceAll("[,.]", "");
		text = text.replace("\"", "");
		text.replaceAll("&nbsp", " ");
		List<String> textClean = stemming(text);
		// want to emphasize the priority of title, double the title word count
		if (titleClean != null) {
			textClean.addAll(new ArrayList(titleClean));
			textClean.addAll(new ArrayList(titleClean));
		}
		return textClean;
	...
	//remove stopwords
	public List<String> stemming(String s) {
		String[] stopwords = { "a", "as", "able", "about", "above", "according", "accordingly", "across", "actually",
				...
				"hadnt", "happens", "hardly", "has", "hasnt", "have", "havent", "having", "he", "hes", "hello", "help",
				"hence", "her", "here", "heres", "hereafter", "hereby", "herein", "hereupon", "hers", "herself", "hi",
				"him", "himself", "his", "hither", "hopefully", "how", "howbeit", "however", "i", "id", "ill", "im",
				"ive", "ie", "if", "ignored", "immediate", "in", "inasmuch", "inc", "indeed", "indicate", "indicated",
				"indicates", "inner", "insofar", "instead", "into", "inward", "is", "isnt", "it", "itd", "itll", "its",
				"its", "itself", "just", "keep", "keeps", "kept", "know", "knows", "known", "last", "lately", "later",
				...
				"while", "whither", "who", "whos", "whoever", "whole", "whom", "whose", "why", "will", "willing",
				"wish", "with", "within", "without", "wont", "wonder", "would", "would", "wouldnt", "yes", "yet", "you",
				"youd", "youll", "youre", "youve", "your", "yours", "yourself", "yourselves", "zero", "i'm", "you're",
				"they're", "it's" };
		Set<String> stopSet = new HashSet<String>(Arrays.asList(stopwords));
		ArrayList<String> tokens = new ArrayList<String>(Arrays.asList(s.split(" ")));
		for (int i = 0; i < tokens.size(); i++) {
			String token = tokens.get(i);
			if (stopSet.contains(token.trim()) || token == null) {
				tokens.remove(i);
				i--;
			}
		}
		return tokens.stream().map(str -> str.trim()).collect(Collectors.toList());
